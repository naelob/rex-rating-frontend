...
rule cannotReviewTwice(address user, uint restaurantId, string reviewText, uint rating) {
    ...
    assert before_nb_reviews == after_nb_reviews, "User can't review twice the same restaurant";
}
...
rule checkStorageIsUpdated(
    ...
    assert total_restaurants +1 == total_updated_restaurants, "Restaurants must be updated by one restaurant";
}
