...
rule cannotReviewTwice(address user, uint restaurantId, string reviewText, uint rating) {
    assume restaurantId < restaurants.length; // Restaurant exists.
    assume rating >= 0 && rating <= 5; // Valid rating.
    assume !hasReviewed[user][restaurantId]; // User has not reviewed this restaurant yet.
